...
function roll_angle_entry_exit(_angle)
   ...
   function self.get_roll(t, time_s)
      if AEROM_ENTRY_RATE:get() * 0.5*time_s < angle then
         gcs:send_text(0,string.format("WARNING: Requested roll rate too slow: rate of %.2f deg/s cannot reach %d deg in %.2f seconds.", AEROM_ENTRY_RATE:get(), angle, 0.5*time_s))
      end
